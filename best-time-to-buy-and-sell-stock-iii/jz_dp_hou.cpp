...
public:
    int maxProfit(vector<int>& prices) {
        ...
        f[0][1] = 0;
        for (int j = 2; j <=5; ++j) f[0][j] = INT_MIN; //Integer.MIN_VALUE;
        for (int i = 1; i <= n; ++i) {
...
//accepted first sub, runtime 12ms, faster than 47%, memory less than 92%
class Solution {
public:
    int maxProfit(vector<int>& prices) {
        if (prices.empty()) return 0;
        int n = prices.size();
        //int [][] f = new int[n+1][5+1];
        vector<vector<int>> f(2, vector<int>(5+1));
        f[0][1] = 0;
        for (int j = 2; j <=5; ++j) f[0][j] = INT_MIN; //Integer.MIN_VALUE;
        int prev = 0, cur = 1;
        for (int i = 1; i <= n; ++i) {
        ...
        return max(f[prev][1], max(f[prev][3], f[prev][5]));
    }
}; //c++ required
