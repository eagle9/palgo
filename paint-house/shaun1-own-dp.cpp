...
//cutting: paint to house i with r/g/b, 3 arrays to store the min cost of paiting to house i
class Solution1 {
public:
    int minCost(vector<vector<int>>& costs) {
        ...
        b[0] = costs[0][2];
        for (int i = 1; i < nh; ++i) {
			//paint to house i with red, derive from min cost painting to house i-1 with either green or blue, then paint house i with red -- cost[i][0]
        	r[i] = min(r[i],costs[i][0] + min(g[i-1],b[i-1]));
        	g[i] = min(g[i],costs[i][1] + min(r[i-1],b[i-1]));
        	b[i] = min(b[i],costs[i][2] + min(r[i-1],g[i-1]));
        }
		//the min cost of paiting to last house, index = nh-1, 
		//3 possibilities, r or g or b, take the min
