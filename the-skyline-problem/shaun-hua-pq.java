...
class Solution {
    public List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        for (int[] a : buildings) {
            ...
            int [] e1 = {a[0],-a[2]};
            events.add(e1);
            events.add(new int[]{a[1], a[2]});
        }
        //给height按照左边界排序，这样我们就可以按顺序来处理那些关键的节点了
        //according to x, break x tie with height
        ...
        */
        ...
        for(int[] event:events) {
            if(event[1] < 0) { //entering
                if (-event[1] > pq.peek()) {
                    res.add(Arrays.asList(event[0], -event[1]));
                }
                pq.offer(-event[1]);
            } else { //leaving
                pq.remove(event[1]);
                if (event[1] > pq.peek()) {
                    res.add(Arrays.asList(event[0], pq.peek()));
                }
            }
