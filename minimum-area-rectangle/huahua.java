class Solution {
    ...
    //runtime 522ms, faster than 18%, huahua's idea to use Map<Integer,Set<Integer>>, a little bit faster than using Set<String>
    public int minAreaRect(int[][] points) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = i+1; j < N; j++) {
                ...
                if (map.containsKey(p1[0]) && map.get(p1[0]).contains(p4[1]) && map.containsKey(p4[0]) && map.get(p4[0]).contains(p1[1])) {
