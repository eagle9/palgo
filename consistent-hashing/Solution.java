...
public class Solution {
    ...
    public List<List<Integer>> consistentHashing(int n) {
        ...
        newMachine.add(0); //key starting value
        newMachine.add(359); //key end value
        newMachine.add(1); //server machine id
        ret.add(newMachine);
       
	    //simulate the process of adding more machines for horizontal sharding	
        for (int i = 1; i < n; i++) {
            newMachine = new ArrayList<Integer>();
            int max = Integer.MIN_VALUE; //max init with small number
            List<Integer> target = ret.get(0); //target init with first one
           
		    //find the max key interval from current ret	
            for (int j = 0; j < ret.size(); j++) {
                List<Integer> range = ret.get(j);
				int start = range.get(0), end = range.get(1);
                if (end - start > max) { //why +1
                    max = end - start;
                    target = range;
                }
            }
           
		    //what does this mean? i see, too many machines,now ranges is too small	
            if (max == 0) {
                return ret;
            }
           
		    //divide target into two ranges	
			int start = target.get(0),end = target.get(1);
			int mid = start + (end - start)/2;
            newMachine.add(mid + 1);
            newMachine.add(end);
            newMachine.add(i + 1); //new machine id
            target.set(1, mid);
            ret.add(newMachine);
        }
