...
/*
       ...
       dp[i-1][j]    dp[i][j-1]       dp[i-1][j-1], no x or y
       1 1 1                          1 1 1  x
       1 1 1         1 1 1            1 1 1  x   ===> dp[i-1][j], no y
       1 1 1         1 1 1            1 1 1  x
          <1>        1 1 1 <1>        y y y  <1>:ij  
                                        ||
                                        V
                                        dp[i][j-1]
          if matrix[i][j] == 1, the 3 rectangles 
            dp[i][j] = min(a, b, c) + 1
...
public:
    int maximalSquare(vector<vector<char>>& matrix) {
        ...
        for (int i = 0; i < m; ++i)
            for (int j = 0; j < n; ++j) {
                if (i == 0 || j == 0) {
                    dp[i][j] = matrix[i][j] - '0'; //at border, 1 if cell value is 1, otherwise 0
