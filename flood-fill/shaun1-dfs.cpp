...
color == newColor will lead to dfs no exit, beware
after fixing this, accepted
runtime 24ms, faster than 77%, mem less than 78%
lessons:
1) visited or not? 
2) prevent recursion no exit
3) read problem requirements carefully
*/
...
public:
    ...
    vector<vector<int>> floodFill(vector<vector<int>>& image, int sr, int sc, int newColor) {
        ...
        if (newColor == image[sr][sc]) return res; //bug missing this one leads stack overflow
